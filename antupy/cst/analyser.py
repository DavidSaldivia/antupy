...
DIR_MAIN = os.path.dirname(DIR_FILE)
...
def parametric__runs_paper() -> pd.DataFrame:

    sim_base = CSTSimulator()
    setattr(sim_base.devices["solar_field"], "Npan", 16)
    setattr(sim_base.devices["solar_field"], "Ah1", 7.07*7.07)
    lat   = -23.
    ...
    parametric = ParametricAnalyser(
        ...
        sim_base = sim_base,
        dir_results = 'results_paper',
        file_results = "results.csv",
        show_plots = False,
        dir_data = os.path.join(DIR_MAIN, 'data\mcrt_datasets_paper'),
    )
    runs = parametric.analysis()
    return runs


def parametric_runs_thesis() -> pd.DataFrame:
    sim_base = CSTSimulator()
    setattr(sim_base.devices["solar_field"], "Npan", 1)
    setattr(sim_base.devices["solar_field"], "Ah1", 2.92*2.92)
    type_shdw='simple'
    Npan  = 1
    lat   = -23.
    parametric = ParametricAnalyser(
...
def parametric_runs_point() -> pd.DataFrame:
    sim_base = CSTSimulator()
    setattr(sim_base.devices["solar_field"], "Npan", 1)
    setattr(sim_base.devices["solar_field"], "Ah1", 2.92*2.92)
    type_shdw='simple'
...
def parametric_runs_power_variable() -> pd.DataFrame:
    sim_base = CSTSimulator()
    setattr(sim_base.devices["solar_field"], "Npan", 1)
    setattr(sim_base.devices["solar_field"], "Ah1", 2.92*2.92)
    type_shdw='simple'
